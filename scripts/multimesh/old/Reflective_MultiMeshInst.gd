...
func texture_setup():
	...
	multimesh.mesh = $"Reflective_Mold".mesh.duplicate()
	...
	print("Reflective Nebulae = ", stars.size()," Intended = ", n_of_instances)
	multimesh.instance_count = stars.size()
	
	for i in range(stars.size()):
		var tf = stars[i][0]
		var ctm = stars[i][1]
		var clr = Color(randf(), 0.0*randf(), 0.0*randf(), randf())
